...
// ============================================
// 5. CRUD — Conversations
// ============================================

export function createConversation(title: string = 'Untitled'): string {
  ...
  db.runSync(
    `INSERT INTO conversations (id, user_id, title, created_at, updated_at, is_synced, device_id)
     VALUES (?, ?, ?, ?, ?, 0, ?)`,
    [id, userId, title, now, now, deviceId]
  );
...
export function createMessage(conversationId: string, content: string): string {
  ...
  db.runSync(
    `UPDATE conversations SET updated_at = ?, is_synced = 0 WHERE id = ?`,
    [now, conversationId]
  );
...
export function editMessage(messageId: string, newContent: string) {
  const now = new Date().toISOString();
  db.runSync(
    `UPDATE messages SET content = ?, updated_at = ?, is_synced = 0 WHERE id = ?`,
...
export function deleteMessage(messageId: string) {
  const now = new Date().toISOString();
  db.runSync(
    `UPDATE messages SET deleted_at = ?, updated_at = ?, is_synced = 0 WHERE id = ?`,
    [now, now, messageId]
  );
}

// ============================================
// 7. EXPORT THE DB (for sync layer later)
// ============================================
export { db };
